...
- 現在の問い: 3レイヤーのルーパーと直接触れる合成波形に、KAOSSの演奏性を複雑化させず統合するにはどうするか
...
MIC / LIVE INPUT
   ...
   INPUT GAIN
        |
  LOOP 1 --+
  LOOP 2 --+--> layer / MIX waveform performance --> KAOSS MASTER --> final space --> OUTPUT GAIN
  LOOP 3 --+
```

- 中央の波形面は、録音素材の位置と断片を直接操作する。
- 右側のKAOSS面は、3レイヤー合成後の全体を処理する。
- レイヤー選択は中央の `1 / 2 / 3 / MIX` に置く。
...
| 左 | 中央 | 右 |
| --- | --- | --- |
| LOOP 1〜3のREC / PLAY / STOP | 大きな合成波形 | KAOSS XY |
| 各レイヤーの小波形と状態 | `1 2 3 MIX` | プリセット |
...
5. 「縦画面中心の一画面」  
   合成波形、3レイヤー、KAOSSマスターを同時に直接操作するため横画面へ変更。
...
- 3レイヤー波形描画とリアルタイムグラニュラー処理を同時実行した際のCPU負荷。
