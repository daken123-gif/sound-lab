...
## 2026-09-02追補 — Aphex Twin研究ブランチの直接取得とv2接続

### 「本文未取得」判定の訂正

Section 18、最初のSection 20、Section 23に残る「Aphex Twin研究は本文未取得」「対応branch headなし」という記述は、現在状態として失効した。探索履歴として旧記述は残すが、判断には使わない。

| research-id | 取得ref | 取得ファイル | blob SHA | 現在状態 |
| --- | --- | --- | --- | --- |
| 20260902-aphex-twin | `research/20260902-aphex-twin` | `research/20260902-aphex-twin/README.md` | `49e578cadfcd6d6ce4e3166279e8ae951cc3d9fb` | PRIMARY_ARTIFACTとして本文取得 |

ブランチ検索結果は検索語そのものを含むため所在確認だけに使い、研究上の接続は上記refから直接取得したREADME本文に基づけた。

### DillaとAphex Twinの軸を分ける

Aphex Twin研究からDilla研究へ接続するのは、作家様式の表面的な類似ではなく、**何が同一性を保持し、何が別の時間幅で変形するか**という比較軸である。

| 軸 | J Dilla研究 | Aphex Twin研究 |
| --- | --- | --- |
| 主な問い | 声部が安定層、細分、attack、小節位置に対してどこへ置かれるか | 旋律、低域、音色族、休符形、調律関係の何が局所編集や別takeを越えて残るか |
| 決定単位 | onset、声部間差、周期内のoffset shape | eventだけでなく句、録音take、再聴取、別版 |
| 現在の証拠 | 先行研究、短いpreview周期、合成反例。独自フル尺onset測定は未完了 | 一次資料を含む取得済み研究本文。計画された曲別聴取・実測は未完了 |
| 混同しないもの | 「遅いsnare」や一括swingへの単純化 | 「全部手編集」「全部生成」「一回録音＝準備なし即興」への単純化 |

Dillaの時間関係をAphex Twinの編集方法で説明したことにはしない。逆に、Aphex Twinの曲内変形をDillaのmicrotimingへ回収しない。

### 時間場モデルv2

`data/cross-research-timing-model-v2.json` を追加し、v1の六操作を保持したまま次の軸を増やした。

- `select_identity_anchor`: 変形中にも識別可能であるべき要素を演奏者が明示的に選ぶ
- `transform_local_detail`: 選択した時間幅の内部だけを変形する
- `commit_snapshot`: ユーザーの明示操作時だけ現在演奏を記録し、自動再生へ移らない
- `relisten_then_reenter`: 明示的に選んだ記録を次の演奏判断へ返し、システムが完成takeを選ばない

追加した`identity_memory`は、完成済みloopを自動保持する機構ではない。保持候補は句形、低域位置、音色族、accent／休符形、調律関係であり、録音開始、再生、take選択は自動化しない。

### v2の反証条件

1. 局所event統計を保ったままidentity anchorを除いても、連続性が変わらない。
2. identity anchorを保って局所順序を変えた場合にも、方向感が保たれない。
3. 明示snapshotより自動loop再生の方が、演奏者の現在の判断を損なわない。
4. event単位の回復と句単位の再入場を区別しても、聴取・操作上の差が出ない。
