...
作業領域の自動削除後、解析コードをGitの研究ブランチから復元した。音声はShazamのApple Music song ID `1660486400`と`1660367654`から公式preview URLを再取得した。

再取得AACのSHA-256は以前の記録と一致した。

- Stem / Long Stem: `12f10578146558fd2e15d8342314b8f1a155481522ec514e40887c77d00264a7`
- Stem (Cops 'n' Robbers): `317551ae1ad70813f818fbe1ae7c7a4e101cf36c3ef1980e88444f916d3f9b29`

校正器は12項目中12項目が再通過した。以前の音声と解析条件を復元できたことは確認したが、これはフル尺取得を意味しない。

### 33.2 事前に固定した13構成

候補event indexと8190対照組は変えず、特徴量だけを変更した。

| 特徴構成 | 次元 | 候補順位 | 候補より弱い組 |
| --- | ---: | ---: | ---: |
| 4帯域＋centroid＋RMS | 6 | 185 / 8190 | 97.74% |
| 4帯域のみ | 4 | 1610 / 8190 | 80.34% |
| 4帯域＋centroid | 5 | 626 / 8190 | 92.36% |
| 4帯域＋RMS | 5 | 598 / 8190 | 92.70% |
| 8帯域＋centroid＋RMS | 10 | 580 / 8190 | 92.92% |
| 8帯域のみ | 8 | 2014 / 8190 | 75.41% |
| 8帯域＋centroid | 9 | 1575 / 8190 | 80.77% |
| 8帯域＋RMS | 9 | 1024 / 8190 | 87.50% |
| 12帯域＋centroid＋RMS | 14 | 591 / 8190 | 92.78% |
| 12帯域のみ | 12 | 2077 / 8190 | 74.64% |
| 12帯域＋centroid | 13 | 1849 / 8190 | 77.42% |
| 12帯域＋RMS | 13 | 937 / 8190 | 88.56% |
| centroid＋RMSのみ | 2 | 21 / 8190 | 99.74% |

13構成中、95 percentile以上は2構成、90 percentile以上は6構成だった。帯域分割を細かくしても順位は上がらず、帯域比のみでは74.64–80.34 percentileへ落ちた。したがって前節で観測したtrajectory上位性を、詳細なスペクトル形状の一致とは読めない。

### 33.3 事後診断

13構成の結果を見た後、centroidとRMSのどちらが支配するかを単独で測った。この二構成は事後追加であり、事前固定群へ混ぜない。

| 事後構成 | trajectory cosine | 順位 | 候補より弱い組 |
| --- | ---: | ---: | ---: |
| centroidのみ | 0.8620 | 17 / 8190 | 99.79% |
| RMSのみ | 0.7841 | 157 / 8190 | 98.08% |

最大の寄与はcentroid、次がRMSだった。ここでcentroidは知覚上の「明るさ」に関係する粗い重心量であり、音色、楽器、sample identityを単独で表さない。

### 33.4 前節の判断を訂正する

前節では、9イベント列について「時間骨格と粗いイベント特徴の両方が稀に近い」「相対的なスペクトル・RMS変化の順序も対照の上位2.26%」と記した。6次元構成内の数値としては正しいが、特徴量を交換した後も維持される一般的な音色系列の一致であるかのように広げることはできない。

訂正後の観測は次である。

> 二版の局所9イベント列は、イベント間隔、spectral centroidの推移、RMSの推移が強く近い。一方、4・8・12分割の帯域比trajectoryは特異性が弱く、詳細なスペクトル系列の一致は確認できない。
...
local_event_sequence:
  timing_specificity: strong_within_preview_control
  centroid_trajectory: strong
  rms_trajectory: elevated
  band_ratio_trajectory: not_robust
  shared_rhythmic_energy_contour: strengthened
  shared_timbre_sequence: unsupported
  sample_identity: unacquired
```

「局所的な共通イベント単位」仮説は時間骨格と明るさ・強度包絡の範囲で残る。「音色系列の共有」へは進めない。同一素材でなくても、同じリズム配置へ異なる音を置き、明るさと強度を似た順序で動かせば今回の測定値は生じうる。

### 33.5 追加保存と次の工程

追加保存:

- `event-feature-ablation-20260905.json`
- `tools/event_feature_ablation.py`

次はcentroid一致の原因を分ける。候補9イベントそれぞれについて、centroidの絶対値、変化方向、RMSとの共変動を列として出し、単なる全体的な明暗・強弱カーブか、イベント固有の交替順序かを検査する。聴取、フル尺、sample identity、編集系譜は未取得のまま保持する。
