...
export function normalizeContactGestureFrame(input) {
  ...
  validateKeys(input);
  if (input.schemaVersion !== SCHEMA_VERSION) reject("SCHEMA_VERSION", `schemaVersion must be ${SCHEMA_VERSION}`);
  ...
  if (!Number.isInteger(input.pointerId) || input.pointerId < 0) reject("POINTER_ID", "pointerId must be a non-negative integer");
  if (!PHASES.has(input.phase)) reject("PHASE", `Unknown phase: ${input.phase}`);
  if (!PRESSURE_SOURCES.has(input.pressureSource)) reject("PRESSURE_SOURCE", `Unknown pressureSource: ${input.pressureSource}`);
...
export class ContactGestureGate {
  ...
  accept(input) {
    ...
    if (!state) {
      if (frame.phase !== "contact") reject("PHASE_START", "A gesture sequence must start with contact");
    ...
    if (frame.timestampMs < state.timestampMs) reject("TIMESTAMP_REGRESSION", "timestampMs cannot move backwards within a gesture");
