...
## 2026-08-28 ANALOG部分メイクアップ

- ANALOGコンプレッサーの実ゲインリダクションの一部を、DENSITYに応じて後段で戻す。
- DENSITY 35%、6 dBリダクション時は +0.735 dB。
- DENSITY 100%、6 dBリダクション時は +2.1 dB。
- 最大値は +2.5 dBに固定し、圧縮を打ち消す全量メイクアップは行わない。
- 120 msで追従させ、音節ごとの急なゲイン変動を抑える。
- CLEAN経路と4トラック構成は変更しない。
- iPhone実機でのCLEAN/ANALOG知覚音量一致は未検証。
